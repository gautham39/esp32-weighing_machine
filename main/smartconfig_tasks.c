                                ...
                                int32_t event_id, void* event_data)
{
    ...
    /*here after entering data in esp touch app retrive the data and initialize and store data in wifi config ,
    ...
    esp_err_t err = nvs_open("wifi_storage", NVS_READWRITE, &nvs_storage);
    if (err != ESP_OK) {
        printf("Error opening NVS\n");
        return;
    }
     nvs_set_str(nvs_storage, "ssid", ssid);
    nvs_set_str(nvs_storage, "password", password);
    nvs_commit(nvs_storage);
    nvs_close(nvs_storage);

    printf("Wi-Fi credentials stored in NVS\n");
